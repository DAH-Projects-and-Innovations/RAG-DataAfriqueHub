...
class RAGPipelineFactory:
    ...
    def load_config(cls, config_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Configuration invalide dans '{config_path}':\n{errors}") from exc
    ...
    def _create_component(cls, component_type: str, component_config: dict[str, Any]):
        ...
        if name not in cls._registry[component_type]:
            ...
            raise ValueError(
                f"Composant non enregistré: {component_type}/{name}. Disponibles: {available}"
            )
    ...
    @classmethod
    def list_components(cls, component_type: str | None = None) -> dict[str, list]:
        """
