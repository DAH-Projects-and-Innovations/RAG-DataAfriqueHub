...
class RAGPipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"Pipeline RAG initialisé avec: "
    ...
    def ingest(self, loader: IDocumentLoader, chunker: IChunker, source: str, **kwargs) -> int:
        """
    ...
    def query(
        self, query_text: str, top_k: int = 5, rerank_top_k: int | None = None, **kwargs
    ) -> RAGResponse:
        """
        ...
        metadata = {"original_query": query_text, "steps": [], "top_k": top_k}
        ...
        if self.query_rewriter:
            try:
                queries = self.query_rewriter.rewrite(
                    query_text, **kwargs.get("rewriter_params", {})
                )
        ...
        for query in queries:
            ...
            try:
                ...
                docs = self.retriever.retrieve(
                    query_obj, top_k=top_k, **kwargs.get("retriever_params", {})
                )
        ...
        try:
            answer = self.llm.generate_with_context(
                query_text, unique_docs, **kwargs.get("llm_params", {})
            )
        ...
        return RAGResponse(answer=answer, sources=unique_docs, metadata=metadata, query=query_text)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "retriever": {"type": type(self.retriever).__name__},
                "llm": {"type": type(self.llm).__name__},
                "optional_components": {
