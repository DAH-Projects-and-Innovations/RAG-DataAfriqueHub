...
class RetrievalComparator:
    ...
    def run_comparison(self, top_k_values: list[int] | None = None) -> dict[str, RetrievalMetrics]:
        """
        ...
        for strategy_name, strategy in self.strategies.items():
            ...
            metrics = self._evaluate_strategy(strategy_name, strategy, top_k_values)
    ...
    def _evaluate_strategy(
        self, name: str, strategy: RetrievalStrategy, top_k_values: list[int]
    ) -> RetrievalMetrics:
        """
        ...
        for query in self.queries:
            ...
            for k in top_k_values:
                ...
                # Recall@k
                recall = relevant_at_k / len(relevant_docs) if len(relevant_docs) > 0 else 0
        ...
        return RetrievalMetrics(
            ...
            precision_at_k={k: statistics.mean(v) if v else 0 for k, v in precision_at_k.items()},
            recall_at_k={k: statistics.mean(v) if v else 0 for k, v in recall_at_k.items()},
            mrr=statistics.mean(reciprocal_ranks) if reciprocal_ranks else 0,
            ndcg_at_k={k: statistics.mean(v) if v else 0 for k, v in ndcg_at_k.items()},
        )

    def _compute_reciprocal_rank(self, retrieved: list[str], relevant: set) -> float:
        """
    ...
    def print_comparison_table(self, results: dict[str, RetrievalMetrics]) -> None:
        """
        ...
        print(
            f"{'Latency (ms)':<20} "
            + " ".join(f"{results[s].avg_latency_ms:<15.2f}" for s in strategies)
        )

        # MRR
        print(f"{'MRR':<20} " + " ".join(f"{results[s].mrr:<15.4f}" for s in strategies))

        # Precision@k
        for k in [1, 3, 5, 10]:
            print(
                f"{'Precision@' + str(k):<20} "
                + " ".join(f"{results[s].precision_at_k[k]:<15.4f}" for s in strategies)
            )
        ...
        for k in [5, 10]:
            print(
                f"{'NDCG@' + str(k):<20} "
                + " ".join(f"{results[s].ndcg_at_k[k]:<15.4f}" for s in strategies)
            )

        print("\n" + "=" * 80)


def create_synthetic_test_data() -> tuple[list[Document], list[Query], dict[str, list[str]]]:
    """
