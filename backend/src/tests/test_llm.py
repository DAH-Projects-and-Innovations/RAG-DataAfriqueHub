...
def load_llm_class(filename, class_name, extra_mocks=None):
    ...
    source = source.replace(
        "from ..core.interfaces import ILLM", "from src.core.interfaces import ILLM"
    )
    source = source.replace(
        "from ..core.models import Document", "from src.core.models import Document"
    )
...
class TestMistralLLM(unittest.TestCase):
    ...
    def test_03_generate_with_context(self):
        ...
        response = self.llm.generate_with_context(query="Qu'est-ce que le RAG ?", context=docs)
        self.assertIsInstance(response, str)
        self.assertGreater(len(response), 10)
    ...
    def test_04_anti_hallucination(self):
        ...
        response = self.llm.generate_with_context(
            query="Quel est le prix du Bitcoin aujourd'hui ?", context=docs
        )
        self.assertIsInstance(response, str)
