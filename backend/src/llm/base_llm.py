...
class OpenAILLM(BaseLLM):
    ...
    def generate(self, messages: list[LLMMessage], **kwargs) -> LLMResponse:
        ...
        # Appeler l'API
        response = self._client.chat.completions.create(messages=messages_dict, **params)
...
class AnthropicLLM(BaseLLM):
    ...
    def generate(self, messages: list[LLMMessage], **kwargs) -> LLMResponse:
        ...
        # Appeler l'API
        response = self._client.messages.create(messages=conversation_messages, **params)
    ...
    def generate(self, messages: list[LLMMessage], **kwargs) -> LLMResponse:
        ...
        for msg in messages:
            if msg.role == "system":
                ...
                request_messages.append({"role": "user", "parts": [{"text": msg.content}]})
            elif msg.role == "assistant":
                request_messages.append({"role": "model", "parts": [{"text": msg.content}]})
        ...
        try:
            ...
            response = self._client.models.generate_content(
                model=model_name, contents=request_messages, config=config
            )
            ...
            raise RuntimeError(f"Erreur lors de l'appel Gemini API: {str(e)}") from e
...
class MistralLLM(BaseLLM):
    ...
    def generate(self, messages: list[LLMMessage], **kwargs) -> LLMResponse:
        ...
        # Appel API
        response = self._client.chat.complete(messages=messages_dict, **params)
...
def create_llm(provider: str | LLMProvider, model: str, **kwargs) -> BaseLLM:
    """
