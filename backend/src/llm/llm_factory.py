...
def create_llm_adapter_from_config(
    ...
    **kwargs,
    ...
    llm_kwargs = {"temperature": temperature, "max_tokens": max_tokens, "top_p": top_p}
    ...
    # Créer l'adaptateur
    return LLMAdapter(llm=llm, prompt_manager=prompt_manager, config=rag_config)
...
class LLMAdapterFactory:
    ...
    def __init__(self, provider: str, model: str, prompt_manager: PromptManager, **kwargs):
        """
        ...
        self.adapter = create_llm_adapter_from_config(
            provider=provider, model=model, prompt_manager=prompt_manager, **kwargs
        )
