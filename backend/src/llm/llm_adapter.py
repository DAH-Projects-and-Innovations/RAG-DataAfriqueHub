...
class LLMAdapter(ILLM):
    ...
    def generate_with_context(self, query: str, context: list[Document], **kwargs) -> str:
        ...
        sources = self._format_sources(
            context, kwargs.get("include_citations", self.config.enable_citations)
        )
        ...
        try:
            if kwargs.get("include_citations", self.config.enable_citations):
                system_prompt = self.prompt_manager.render_template(
                    system_template, sources=sources
                )
            else:
                system_prompt = self.prompt_manager.render_template(
                    system_template, context=sources
                )

            user_prompt = self.prompt_manager.render_template(user_template, question=query)
        except Exception:
...
def create_llm_adapter(
    ...
    **llm_kwargs,
    ...
    # Créer l'adaptateur
    return LLMAdapter(llm=llm, prompt_manager=prompt_manager, config=config)
