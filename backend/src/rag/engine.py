...
class RAGEngine:
    ...
    def query(self, query: RAGQuery, **kwargs) -> RAGResponse:
        ...
        confidence = self._evaluate_confidence(llm_response.content, sources, citations)

        # 7. Détecter les réponses hors contexte
        based_on_context = self._verify_context_usage(llm_response.content, sources)
    ...
    def _retrieve_documents(self, query: RAGQuery) -> list[Document]:
        ...
        Returns:
            Documents récupérés
        """
        retrieval_query = Query(text=query.question, filters=query.retrieval_filters)

        documents = self.retriever.retrieve(query=retrieval_query, top_k=query.top_k)
    ...
    def _format_context(self, sources: list[Source], include_citations: bool) -> str:
        """
    ...
    def _generate_answer(
        ...
        # Rendre les prompts
        if include_citations:
            system_prompt = self.prompt_manager.render_template(system_template, sources=context)
        else:
            system_prompt = self.prompt_manager.render_template(system_template, context=context)

        user_prompt = self.prompt_manager.render_template(user_template, question=question)
        ...
        # Générer
        return self.llm.generate(messages=messages, temperature=temperature, max_tokens=max_tokens)

    def _extract_citations(self, answer: str, sources: list[Source]) -> list[Citation]:
        """
        ...
        for match in matches:
            ...
            source = next((s for s in sources if s.citation_id == citation_num), None)
    ...
    def _evaluate_confidence(
        self, answer: str, sources: list[Source], citations: list[Citation]
    ) -> ConfidenceLevel:
        """
    ...
    def _verify_context_usage(self, answer: str, sources: list[Source]) -> bool:
        """
