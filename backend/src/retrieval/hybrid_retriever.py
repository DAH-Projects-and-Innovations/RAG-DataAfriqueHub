...
class HybridRetriever(IRetriever):
    ...
    def retrieve(
        ...
        dense_results = self.dense_retriever.retrieve(
            query, top_k=self.config.top_k_per_retriever, filters=filters
        )
        ...
        if self.config.fusion_strategy == "weighted_sum":
            fused_results = self._weighted_sum_fusion(dense_results, bm25_results)
            ...
            fused_results = self._max_fusion(dense_results, bm25_results)
        else:
    ...
    def _weighted_sum_fusion(
        self, dense_results: list[Document], bm25_results: list[Document]
    ) -> list[Document]:
        """
        ...
        for doc in dense_results:
            score = doc.metadata.get(
                "normalized_score" if self.config.normalize_scores else "score", 0
            )
            doc_scores[doc.id]["doc"] = doc
            doc_scores[doc.id]["scores"].append(("dense", score * self.config.dense_weight))

        # Ajouter les scores BM25
        for doc in bm25_results:
            score = doc.metadata.get(
                "normalized_score" if self.config.normalize_scores else "bm25_score", 0
            )
            if doc_scores[doc.id]["doc"] is None:
                doc_scores[doc.id]["doc"] = doc
            doc_scores[doc.id]["scores"].append(("bm25", score * self.config.bm25_weight))

        # Calculer les scores finaux
        fused_docs = []
        for _doc_id, data in doc_scores.items():
            doc = data["doc"]
            ...
            doc.metadata["fusion_details"] = {name: score for name, score in data["scores"]}
    ...
    def _rrf_fusion(
        ...
        for rank, doc in enumerate(dense_results, start=1):
            ...
            doc_scores[doc.id]["doc"] = doc
            doc_scores[doc.id]["rrf_score"] += rrf_contribution * self.config.dense_weight
        ...
        for rank, doc in enumerate(bm25_results, start=1):
            ...
            if doc_scores[doc.id]["doc"] is None:
                doc_scores[doc.id]["doc"] = doc
            doc_scores[doc.id]["rrf_score"] += rrf_contribution * self.config.bm25_weight
    ...
    def _max_fusion(
        ...
        for doc in dense_results:
            ...
            doc_scores[doc.id] = {"doc": doc, "dense_score": score, "bm25_score": 0}
        ...
        for doc in bm25_results:
            ...
            if doc.id in doc_scores:
                ...
                doc_scores[doc.id] = {"doc": doc, "dense_score": 0, "bm25_score": score}
