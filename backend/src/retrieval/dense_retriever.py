...
from ..core.interfaces import IEmbedder, IRetriever, IVectorStore
...
class DenseRetriever(IRetriever):
    ...
    def retrieve(
        ...
        if self.config.include_metadata_filter and (filters or getattr(query, "filters", None)):
            search_filters = {
                **(filters or {}),
                **(getattr(query, "filters", None) or {}),
            }

        # Recherche vectorielle (le vector store peut retourner des Chunks ou Documents)
        raw_results = self.vector_store.search(
            query_embedding=query_embedding, top_k=k, filters=search_filters
        )
        ...
        if self.config.similarity_threshold is not None:
            normalized = [
                d
                for d in normalized
                if (getattr(d, "score", None) or d.metadata.get("score", 0))
