...
class RerankerConfig:
    ...
    top_k: int | None = None  # None = retourner tous les documents
...
class BaseReranker(IReranker, ABC):
    ...
    def rerank(
        self, query: Query, documents: list[Document], top_k: int | None = None
    ) -> list[Document]:
        """
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        return {"top_k": self.config.top_k, "min_score": self.config.min_score}
...
class CohereReranker(BaseReranker):
    ...
    def _compute_relevance_score(self, query: str, document: str) -> float:
        ...
        response = self._client.rerank(model=self.model, query=query, documents=[document], top_n=1)
    ...
    def rerank(
        ...
        response = self._client.rerank(
            model=self.model, query=query.text, documents=doc_texts, top_n=k
        )
