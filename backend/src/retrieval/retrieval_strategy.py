...
from ..core.interfaces import IEmbedder, IReranker, IRetriever, IVectorStore
...
class RetrievalConfig:
    ...
    dense_top_k: int = 10
    dense_similarity_threshold: float | None = None
...
class RetrievalStrategy(IRetriever):
    ...
    def _get_dense_retriever(self) -> IRetriever:
        ...
        if self._dense_retriever is None:
            ...
            if self.vector_store is None or self.embedder is None:
                raise ValueError("vector_store and embedder required for dense retrieval")
    ...
    def _get_bm25_retriever(self) -> IRetriever:
        ...
        if self._bm25_retriever is None:
            ...
            self._bm25_retriever = BM25Retriever(documents=self.documents, config=bm25_config)
    ...
    def _get_hybrid_retriever(self) -> IRetriever:
        ...
        if self._hybrid_retriever is None:
            ...
            self._hybrid_retriever = HybridRetriever(
                dense_retriever=dense, bm25_retriever=bm25, config=hybrid_config
            )

        return self._hybrid_retriever

    def _get_reranker(self) -> IReranker | None:
        ...
        if self._reranker is None:
            ...
            if self.config.reranker_type == "cross-encoder":
                model = self.config.reranker_model or "cross-encoder/ms-marco-MiniLM-L-6-v2"
                self._reranker = CrossEncoderReranker(model_name=model, config=reranker_config)
                ...
                model = self.config.reranker_model or "rerank-multilingual-v3.0"
                self._reranker = CohereReranker(
                    api_key=api_key, model=model, config=reranker_config
                )
    ...
    def retrieve(
        ...
        documents = retriever.retrieve(
            query=query, top_k=top_k, filters=all_filters if all_filters else None
        )
