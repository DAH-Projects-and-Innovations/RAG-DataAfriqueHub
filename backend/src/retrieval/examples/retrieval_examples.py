...
from src.retrieval import RetrievalConfig, RetrievalMode
...
def example_4_dynamic_config_change():
    ...
    new_config = RetrievalConfig(
        mode=RetrievalMode.HYBRID, hybrid_fusion_strategy="rrf"
    )
...
def example_6_strategy_comparison():
    ...
    strategies_to_compare = [
        ...
        (
            "Hybrid RRF",
            RetrievalConfig(mode=RetrievalMode.HYBRID, hybrid_fusion_strategy="rrf"),
        ),
        (
            "Hybrid + Rerank",
            RetrievalConfig(
...
def example_7_best_practices():
    ...
    practices = [
        ...
        (
            "🔍 Filtres métadonnées",
            "Utilisez les filtres pour restreindre la recherche",
        ),
        (
            "📊 Mesurer toujours",
            "Utilisez le comparator pour des décisions data-driven",
        ),
        (
            "💰 Coût vs Qualité",
            "Cross-encoder (gratuit) vs Cohere (payant mais meilleur)",
        ),
    ]
